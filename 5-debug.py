...
def bfs(matrix, queue, end, count = 0, route = {}):
    ...
    print( "queue:", queue, "count:", count, end)
    next_pos = set()
    path = []
    for cur in queue:
...
def get_path(matrix, box_path, cur):
    ...
    path += bfs(copy.deepcopy(matrix), {cur}, (box_path[0][0] - dirction[0], box_path[0][1] - dirction[1]), route = dictionary)
    if path == None:
        return
...
for p in box_path:
    ...
    if minimum_path == []:
        ...
        minimum_path = result_path[-1]
    print("result_path:", minimum_path)
